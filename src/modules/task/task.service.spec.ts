...
describe('TaskService', () => {
  ...
  describe('findAll', () => {
    ...
    it('nên áp dụng filter assignedTo và status khi được truyền vào query', async () => {
      const query = { page: 2, limit: 5, assignedTo: 3, status: 'IN_PROGRESS' } as any;
  ...
  describe('updateStatus', () => {
    ...
    it('nên cập nhật status và updatedBy, rồi lưu lại', async () => {
      ...
      (repository.findOne as jest.Mock).mockResolvedValue(existingTask);
      ...
      expect(repository.save).toHaveBeenCalledWith(
        expect.objectContaining({ id: 1, status: 'DONE', updatedBy: actorId }),
      );
      expect(result).toEqual({
